...
public class ClearingHouseClientImpl implements ClearingHouseClient {
    ...
    public ClearingHouseClientImpl(ClientProvider clientProvider, ConfigContainer configContainer, DapsTokenProvider dapsTokenProvider, HttpService httpService) {
    ...
    public Response sendLogToClearingHouse(String logMessage) throws ClearingHouseClientException {
        try {
            ...
            return httpService.sendWithHeaders(body, new URI(clearingHouseUrl + Math.abs(secureRandom.nextInt())), Map.of("Authorization", "Bearer " + dapsTokenProvider.provideDapsToken()));
            ...
            LOGGER.warn(e.getMessage(), e);
            throw new ClearingHouseClientException(String.format("Clearing House URI could not be parsed from String: %s!", clearingHouseUrl), e);
        } catch (IOException e) {
            LOGGER.warn(e.getMessage(), e);
            throw new ClearingHouseClientException("Error while serializing LogMessage header or sending the request!", e);
        }
    }
